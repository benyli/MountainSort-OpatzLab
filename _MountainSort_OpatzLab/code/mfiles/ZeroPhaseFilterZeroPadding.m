...
function samples=ZeroPhaseFilterZeroPadding(samples,samplingrate,cutFrequencies)
%% Filter signal using filtfilt with zeroPadding at start and end to prevent ringing effect.
% cutFrequencies = [low_cut high_cut]
...
nZeros=samplingrate*2;
samples = padarray(samples,[0,nZeros]);
...
samples = samples(nZeros+1:end-nZeros);